...
- `POST /private/reservations` — the atomic hold (serializable tx + row locks, re-validate via `detectConflicts`, `409 {conflicts}`, audit).
...
- On success it inserts the `Reservation` (`HELD`, `expiresAt = now + holdMinutes` default 30) + its `ReservationAsset` rows, computes `effectiveStart/End` from the space buffers, and writes the `AuditEntry` (`reservation.hold`) in the same transaction.
...
- `POST /reservations/:id/confirm` transitions `HELD → CONFIRMED`, clears `expiresAt`, writes audit; it is idempotent; an illegal move (confirm a `RELEASED`) → `409 invalid_transition`; if the hold already expired, confirm returns `409 conflict` with the re-detected `Conflict[]` so the AI can re-plan.
