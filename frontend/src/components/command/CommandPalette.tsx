...
export function CommandPalette({
  ...
  // Reset the term whenever the palette reopens so a stale search never lingers.
  useEffect(() => {
    if (open) setQuery('')
  }, [open])

  // Only hit the network once there's something to search for.
  const requests = useRequests(hasQuery ? { q: debounced, pageSize: 5 } : { pageSize: 5 })
  const spaces = useSpaces(hasQuery ? { q: debounced } : {})
  const assets = useAssets(hasQuery ? { q: debounced } : {})
  ...
  const matchedSpaces = hasQuery
    ? spaceRows.filter((s) => s.name.toLowerCase().includes(term))
    : []
  ...
  const groups: { label: string; icon: React.ElementType; items: PaletteItem[] }[] = [
    {
      ...
      items: requestRows.map((r) => ({
        ...
        to: `/requests/${r.id}`,
        label: r.title,
        meta: r.organizerName,
      })),
    },
    {
      label: t('search.groupSpaces'),
      icon: Building2,
      items: matchedSpaces.map((s) => ({
        key: `space-${s.id}`,
        to: `/spaces/${s.id}`,
        label: s.name,
        meta: t('search.spaceFloor', { n: s.floor }),
      })),
    },
    {
      label: t('search.groupAssets'),
      icon: Boxes,
      items: matchedAssets.map((a) => ({
        key: `asset-${a.id}`,
        to: `/inventory/${a.id}`,
        label: a.name,
        meta: a.location,
              ...
              groups.map((g) =>
                g.items.length ? (
                  <div key={g.label} className="mb-1">
                    <p className="px-4 pb-1 pt-2 text-[11px] font-[600] uppercase tracking-[0.05em] text-text-tertiary">
                      {g.label}
                    </p>
                    {g.items.map((item) => (
                      <button
                      ...
                      >
                        ...
                        {item.meta ? (
                          <span className="ml-auto truncate pl-3 text-[12px] text-text-tertiary">{item.meta}</span>
                        ) : null}
                      </button>
                    ))}
                  </div>
