      ...
      {isLoading ? (
        ...
        <>
          <ul className="flex flex-col gap-3.5">
            {requests.map((r) => <RequestCard key={r.id} r={r} t={t} locale={locale} />)}
          </ul>
          {data && (
            <Pager
              page={data.page}
...
function RequestCard({ r, t, locale }: { r: EventRequest; t: ReturnType<typeof useT>; locale: 'al' | 'en' }) {
  ...
  return (
    <li>
      <Link to={`/portal/${r.id}`} className="block rounded-lg border border-border-subtle p-5 transition-colors hover:border-border hover:bg-surface-sunken">
            ...
            <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] text-text-tertiary">
              ...
              {dates.length > 0 && (
                <span className="flex items-center gap-1">
                  ...
                  {dates.map((d) => formatDateRange(d.start, d.end, locale)).join('; ')}
                </span>
              )}
            </p>
          </div>
