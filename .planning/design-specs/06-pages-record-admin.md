...
- **Topbar** (`topbar(nats)`): height `56px`, `borderBottom:1px solid #ECEEF1`, `background:#fff`, `padding:0 20px`, `gap:14px`.
  ...
  - **NATS pill**: height `30px`, `padding:0 12px`, `borderRadius:999px`. Connected → `background:#E9F6EF` (`--success-subtle`), `border:1px solid rgba(26,127,75,.2)`, dot `#1A7F4B` (`--success`) `7px` with `animation:pulse 1.8s ease-in-out infinite`, text `#15613A` (darker success, no token — closest `--success`) `12px/600` "NATS connected". Degraded → `background:#FBF3E0` (`--warning-subtle`), `border:1px solid rgba(154,107,0,.25)`, dot `#9A6B00` (`--warning`) static, text `#7A5500` "NATS degraded".
...
- AppShell topbar has search + NATS Badge + LocaleToggle + user/logout — **no Copilot button**. (NATS pill is a static `Badge tone="info"` "degraded", not the designed connected/degraded pill, but that's shell scope.)
